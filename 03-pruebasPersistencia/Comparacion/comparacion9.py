...
base_dir = "../10m6/"
resultados_dirs = [f"resultados10m6_{i}" for i in range(10, 110, 10)]
prefijos = sorted(next(os.walk(os.path.join(base_dir, resultados_dirs[0])))[1])

def leer_persistencia_csv(path):
    """Lee un archivo CSV y devuelve lista de tuplas (dim, (birth, death)) en float32"""
    df = pd.read_csv(path, dtype={"dimension": np.int32, "birth": np.float32, "death": np.float32})
    df["birth"] = df["birth"].round(5).astype(np.float32)
    df["death"] = df["death"].round(5).astype(np.float32)
    return [(int(row["dimension"]), (row["birth"], row["death"])) for _, row in df.iterrows()]

for prefijo in prefijos:
    print(f"🔄 Procesando prefijo: {prefijo}")
    output_file = f"distancias_wasserstein_10m6_{prefijo}.csv"
    with open(output_file, mode="w", newline="") as f_out:
        ...
        # --- cargar archivos disponibles por carpeta ---
        ...
        for carpeta_a, carpeta_b in combinations_with_replacement(archivos_por_carpeta, 2):
            ...
            for archivo_a, archivo_b, dim in product(archivos_a, archivos_b, [0, 1, 2]):
                # leer CSVs en cada iteración
                diag_a = leer_persistencia_csv(os.path.join(base_dir, carpeta_a, prefijo, archivo_a))
                diag_b = leer_persistencia_csv(os.path.join(base_dir, carpeta_b, prefijo, archivo_b))
                ...
                if len(diag_a_dim) > 0 and len(diag_b_dim) > 0:
                    ...
                    writer.writerow({
                        ...
                        "archivo_b": archivo_b,
                        "dimension": dim,
                        "distancia": np.float32(dist)  # guardar como 32 bits
                    })
