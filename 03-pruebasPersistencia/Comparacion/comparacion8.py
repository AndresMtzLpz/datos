...
def leer_persistencia_csv(path):
    """Lee un archivo CSV y devuelve una lista de tuplas (dim, (birth, death)) en float32"""
...
for prefijo in prefijos:
    ...
    with open(output_file, mode="w", newline="") as f_out:
        ...
        for carpeta_a, carpeta_b in combinations_with_replacement(archivos_por_carpeta, 2):
            ...
            # precargar todos los diagramas en memoria (en float32)
            diag_a_dict = {
            ...
            for archivo_a, archivo_b, dim in product(archivos_a, archivos_b, [0, 1, 2]):
                diag_a_dim = np.array(
                    [(birth, death) for d, (birth, death) in diag_a_dict[archivo_a] if d == dim],
                    dtype=np.float32
                )
                diag_b_dim = np.array(
                    [(birth, death) for d, (birth, death) in diag_b_dict[archivo_b] if d == dim],
                    dtype=np.float32
                )
                ...
                if len(diag_a_dim) > 0 and len(diag_b_dim) > 0:
                    ...
                    writer.writerow({
                        ...
                        "archivo_b": archivo_b,
                        "dimension": dim,
                        "distancia": np.float32(dist)  # también guardar en 32 bits
                    })
