...
def cargar_y_unir_archivos_por_prefijos_limitado_random(directorio, lista_prefijos, limites):
    ...
    for archivo in archivos_filtrados:
        ...
        if n_lineas and len(df_completo) > n_lineas:
            df = df_completo.sample(n=n_lineas, random_state=42)  # aleatorio pero reproducible
        else:
            df = df_completo
...
if __name__ == "__main__":
    ...
    for n in range(10, 110, 10):
        ...
        print("lineas 3m: ",limites_lineas)
        ...
        for prefijo in prefijos_a_usar:
            ...
            # Cargar datos de 5m limitados al número de líneas de 3m (aleatoriamente)
            df_union = cargar_y_unir_archivos_por_prefijos_limitado_random(
                carpeta_5m, [prefijo], limites_lineas
            )
            if df_union.empty:
